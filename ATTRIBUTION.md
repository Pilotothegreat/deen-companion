...
- **Mushaf line breaks:** where the printed page breaks each of its fifteen lines comes from the King Fahd Glorious Quran Printing Complex's 1405H Madinah edition, read through the [Quran.com API](https://api.quran.com) and generated into `assets/mushaf-lines.json` by `scripts/quran/build_lines.py`. Only the line breaks are taken; every word drawn on the page is Tanzil's text.
