...
- **Adhan and iqama notifications.** The adhan plays as alarm audio through a short foreground service, so it is heard through Do Not Disturb and is never truncated the way a channel sound can be. A reminder before the adhan, "Prayed" and snooze actions, per-prayer mute, iqama as an offset or a fixed time, optional silence during prayer, exact alarms when allowed, and automatic rescheduling after reboots, clock changes and time-zone changes.
...
- **Quran.** The 604-page Madinah mushaf in the Uthmanic Hafs script from the Tanzil Project, shipped verbatim and fully vowelled, with The Clear Quran translation by Talal Itani. Page, juz, hizb, rub' al-hizb, manzil and ruku' generated from the same source, the ayah rosette and the sajdah marks, a jump sheet for any surah, juz or page, diacritic-insensitive search, bookmarks, and ayah-by-ayah recitation (Mishary Alafasy, Al-Husary, Abdul Basit) that caches to disk and plays offline, with repeat, speed and a sleep timer.
...
- **Backup.** Export to a file you keep and restore from one. Cloud backup stays off.
