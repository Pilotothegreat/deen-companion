...
- **Quran.** The 604-page Madinah mushaf in the Uthmanic Hafs script from the Tanzil Project, shipped verbatim and fully vowelled, with The Clear Quran translation by Talal Itani. **Set line for line**: every page breaks its fifteen lines exactly where the King Fahd Complex edition breaks them, so a page is the same shape on every phone and at every text size — the thing that makes a mushaf worth memorising from. Page, juz, hizb, rub' al-hizb, manzil and ruku' from the same source; the ayah rosette is drawn with its number inside it, because no font on Android composes U+06DD with the digits that follow. A jump sheet for any surah, juz or page, diacritic-insensitive search, bookmarks, pinch-to-zoom, a span of ayahs on repeat, and ayah-by-ayah recitation (Mishary Alafasy, Al-Husary, Abdul Basit) that caches to disk and plays offline, with speed and a sleep timer. The screen stays awake while you read, the page follows the recitation and scrolls to whatever you searched for, and after Isha it comes down a shade until Fajr. Above a large system text size the page flows instead, because a fifteen-line page cannot be read at twice the type.
